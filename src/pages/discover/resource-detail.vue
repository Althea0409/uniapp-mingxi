...
function loadResource(id: string) {
  ...
  if (!item) {
    // 尝试按课程ID-资源ID解析并构建真实资源
    const parts = id.split('-');
    if (parts.length >= 2) {
      const cid = parts[0];
      const rid = parts.slice(1).join('-');
      const courseList = (coursesJson as any).courses || [];
      const c = courseList.find((x: any) => x.id === cid);
      if (c) {
        const subject = detectSubject(c.name);
        const pd: any = (portraitData as any)[subject];
        const r = pd?.resources?.find((x: any) => String(x.id) === String(rid));
        if (r) {
          const lows = [...(pd.classicKnowledge || []), ...(pd.modernKnowledge || [])]
            .filter((x: any) => typeof x.value === 'number' && x.value <= 75)
            .slice(0, 2);
